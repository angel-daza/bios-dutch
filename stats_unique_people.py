...
def source_distribution(people: List[MetadataComplete], partition='all'):
    ...
    """
    source_dict_all, source_dict_text = defaultdict(int), defaultdict(int)
...
def create_flask_searchable_data(people: List[MetadataComplete], dataset_filepath: str):
    with open(dataset_filepath, 'w') as fout:
        for p in people:
            ...
            birth_year = p.getBirthDate(method='year_only')[0]
            ...
            save_fields['display_death_year'] = p.getDeathDate(method='year_only')[0]
