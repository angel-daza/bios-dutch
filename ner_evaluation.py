...
def evaluate_bionet_dutch_flair_ner():
    ...
    # OLD SOURCES = ['weyerman', 'knaw', 'vdaa']
    ...
    evaluated_ids = evaluate_ner_outputs(gold_paths, results_path, eval_method="bag_of_entities", eval_title="Original Dutch All", 
...
def evaluate_ner_outputs(gold_paths: List[str], outputs_parent_path: str, eval_method: str,
    ...
    if eval_method == "bag_of_entities":
        ...
        if textid2source:
            ...
            for src in list(sources):
                ...
                print(f"\t{src} --> Precision: {metrics['precision']:.2f}\tRecall: {metrics['recall']:.2f}\tF1 Score: {metrics['f1']:.2f}")
        print("\t----- ALL LABELS ------")
        compute_set_metrics(set(all_gold), set(all_pred), verbose=True)
